...
async function main() {
  ...
  console.log("Categories created:", shirts.nameHe, jackets.nameHe, pants.nameHe, shorts.nameHe);

  const SIZES = ["S", "M", "L", "XL"];

  async function createProduct({ nameHe, price, categoryId, images, sizes }) {
    const existing = await prisma.product.findFirst({ where: { nameHe } });
    if (existing) { console.log("  skip (exists):", nameHe); return; }
    ...
    for (let i = 0; i < images.length; i++) {
      await prisma.productImage.create({ data: { productId: product.id, url: images[i], sortOrder: i } });
    }
    ...
    for (const size of (sizes ?? SIZES)) {
      await prisma.productSize.upsert({
        ...
        update: {},
        create: { productId: product.id, size, stock: 10 },
